// This is the original scratch file where I originally implemented this solution.
// I had a Figma theme built in Figma Token Studio and an updated version of this in a separate theme.
// I wanted to merge the objects deeply, where only items that are different OR if its not present in the target themeObj
// And I wanted the final themeObject to be added to it's own file.
// I used this as an opportunity to learn different ways to merge objects and the limitations of Object.assign and the spread(...) operator
...
const newTheme = deepMerge(WebsiteTheme, DesignSystemTheme);
const newThemeJson = JSON.stringify(newTheme, null, 4);

const fs = require('fs');

const writeJSONStringToFile = (jsonString, path = './') => {
...
writeJSONStringToFile(newThemeJson);


//
// // Function that takes a JSON string and returns a theme object.
// const jsonToTheme = (json) => {
//     const result = {};
//     for (const key of Object.keys(json)) {
//         if (json[key] instanceof Object && !(json[key] instanceof Array)) {
//             result[key] = jsonToTheme(json[key]);
//         } else {
...
//     return result;
// }
// const newThemeObj = jsonToTheme(JSON.parse(newThemeJson));
// console.log("newThemeObj", newThemeObj);
//
// // Function that takes a theme object and returns a JSON string with the theme values and types.
// const themeToCss = (theme) => {
//     const result = {};
//     for (const key of Object.keys(theme)) {
//         if (theme[key] instanceof Object && !(theme[key] instanceof Array)) {
//             result[key] = themeToCss(theme[key]);
//         } else {
//             result[key] = `--${key}: ${theme[key]};`;
//         }
//     }
//     return result;
// }
// const newThemeCss = themeToCss(newTheme);
// console.log("newThemeCss", newThemeCss);
...
//     for (const key of Object.keys(theme)) {
//         if (theme[key] instanceof Object && !(theme[key] instanceof Array)) {
//             result[key] = themeToCssVars(theme[key]);
//         } else {
//             result[key] = `--${key}: ${theme[key]};`;
//         }
//     }
//     return result;
// }
// const newThemeCssVars = themeToCssVars(newTheme);
// console.log("newThemeCssVars", newThemeCssVars);
